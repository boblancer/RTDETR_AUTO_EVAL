...
Outputs: pr_curve.png, iou_distribution.png

Usage:
  python evaluate_detections.py \
      --gt   annotation/run_001/annotations.csv \
      --pred my_video_rtdetr.csv \
      [--iou 0.5]
"""
...
def evaluate(gt_csv, pred_csv, iou_thresh):
    ...
    for frame in all_frames:
        ...
        preds = sorted(pred_map.get(frame, []),
...
def plot_pr_curve(results, mAP, iou_thresh, out_path):
    ...
    ax.text(0.98, 0.06, f"mAP = {mAP:.3f}",
...
def main():
    ...
    results, mAP, ious = evaluate(args.gt, args.pred, args.iou)
    ...
    plot_pr_curve(results, mAP, args.iou, out / "pr_curve.png")
    plot_iou_distribution(ious, args.iou, out / "iou_distribution.png")
    ...
    output = {
        ...
        "iou_stats": {
            "n_matched":   len(ious),
            "mean":        round(float(ious_arr.mean()),   4) if ious else 0.0,
            "median":      round(float(np.median(ious_arr)), 4) if ious else 0.0,
            "min":         round(float(ious_arr.min()),    4) if ious else 0.0,
            "max":         round(float(ious_arr.max()),    4) if ious else 0.0,
            "pct_above_75": round(float((ious_arr >= 0.75).mean()), 4) if ious else 0.0,
        },
    }
